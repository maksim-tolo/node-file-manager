...
export class FileSelector {
    ...
    public unselect(files: Array<File> | File): Array<File> {
        ...
        if (Array.isArray(files)) {
            files.forEach((file) => this.selected.filter((selectedFile) => selectedFile !== file));
        } else {
            this.selected.filter((selectedFile) => selectedFile !== files);
        }
    ...
    private static toggleSelection(files: Array<File> | File): void {
        if (Array.isArray(files)) {
            files.forEach((file) => file.isSelected = !file.isSelected);
        } else {
